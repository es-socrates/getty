document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('/api/announcement')
      ...
      .then((data) => {
        ...
        if (defaultDurationInput)
          defaultDurationInput.value = data.config.defaultDurationSeconds || 10;
  ...
  function renderMessages(messages) {
    ...
    messages.forEach((m) => {
      ...
      const durVal =
        m.durationSeconds || (defaultDurationInput ? Number(defaultDurationInput.value) : 10);
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      if (text.length > 80) return showAlert(t('announcementValidationTooLong'), 'error');
      ...
      fetch('/api/announcement/message', { method: 'POST', body: fd })
        .then((r) => r.json())
        .then((data) => {
  ...
  if (listEl) {
    listEl.addEventListener('click', (e) => {
      ...
      if (action === 'delete') {
        fetch('/api/announcement/message/' + id, { method: 'DELETE' })
          .then((r) => r.json())
          .then(() => loadConfig());
      } else if (action === 'toggle') {
        const enable = btn.textContent === t('inactive');
        fetch('/api/announcement/message/' + id, {
          method: 'PUT',
        ...
        const span = document.querySelector(
          '#announcement-message-form button[type="submit"] span'
        );
        if (span) {
          span.setAttribute('data-i18n', 'announcementEditBtn');
          span.textContent = t('announcementEditBtn');
        }
  ...
  if (saveSettingsBtn) {
    saveSettingsBtn.addEventListener('click', () => {
      ...
      if (isNaN(minutes) || minutes <= 0)
        return showAlert(t('announcementInvalidCooldown'), 'error');
      ...
      fetch('/api/announcement', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          cooldownSeconds: Math.max(60, minutes * 60),
          theme: themeSelect.value,
          bgColor: bgColorInput.value,
          textColor: textColorInput.value,
          animationMode: animationModeSelect ? animationModeSelect.value : undefined,
          defaultDurationSeconds: defaultDur,
          applyAllDurations: applyAll,
        }),
      })
        .then((r) => r.json())
        .then((data) => {
  ...
  if (previewBtn && previewBox) {
    previewBtn.addEventListener('click', () => {
      ...
      const md = raw
        ...
        .replace(
          /\[(.+?)\]\((https?:\/\/[^\s]+?)\)/g,
          '<a href="$2" target="_blank" class="ann-link">$1<\/a>'
        );
      ...
      (e) => {
        if (editIdInput.value) {
          ...
          if (hasNewImage) {
            ...
            fetch('/api/announcement/message/' + id + '/image', { method: 'PUT', body: fd })
              .then((r) => r.json())
              .then((data) => {
                if (!data.success)
                  return showAlert(data.error || t('announcementErrorGeneric'), 'error');
  ...
  try {
    ...
    if (listsCard && !listsCard.querySelector('.ann-clear-buttons')) {
      ...
      btnTest.addEventListener('click', () => {
        ...
        fetch('/api/announcement/messages?mode=test', { method: 'DELETE' })
          .then((r) => r.json())
          .then((d) => {
            if (d.success) {
              showAlert(t('announcementClearedTest') || 'Test messages deleted');
              loadConfig();
      ...
      btnAll.addEventListener('click', () => {
        ...
        fetch('/api/announcement/messages', { method: 'DELETE' })
          .then((r) => r.json())
          .then((d) => {
            if (d.success) {
              showAlert(t('announcementClearedAll') || 'All messages deleted');
              loadConfig();
            } else showAlert(d.error || 'Error', 'error');
          });
      });
