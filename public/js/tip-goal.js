  ...
  const start = () => {
    ...
    function getNonce() {
      try {
        ...
        return (
          (m && (m.nonce || m.getAttribute('nonce'))) ||
    ...
    function setTipGoalVars({ modern, base }) {
      try {
        ...
        if (modern) {
          css +=
            `#goal-widget.modern-theme .modern-card{` +
        ...
        if (base) {
          css +=
            `#goal-widget:not(.modern-theme).tip-goal-widget{` +
        ...
        if (currentData && currentData.customAudioUrl) {
          ...
          try {
            ...
            const url = widgetToken
              ? `/api/goal-audio?widgetToken=${encodeURIComponent(widgetToken)}`
              : '/api/goal-audio';
      ...
      function updateAudioUI() {
        ...
        if (remoteRadio.checked) {
          ...
          audioStatus.textContent =
            '🔊 ' +
            (audioSettings.hasCustomAudio
              ? 'Remote Audio (custom one is saved, but not active))'
              : 'Remote audio');
    ...
    async function loadInitialData() {
      ...
      const widgetToken = getWidgetTokenParam();
      const url = widgetToken
        ? `/api/modules?widgetToken=${encodeURIComponent(widgetToken)}`
        : '/api/modules';
      fetch(url)
        .then((response) => {
    ...
    function scheduleGoalUpdate(data, reason = '') {
      try {
        ...
        updateTimer = setTimeout(() => {
          ...
          updateTimer = null;
          try {
            updateGoalDisplay(d.data, d.reason);
          } catch (e) {
            console.error('updateGoalDisplay failed:', e);
          }
        }, 80);
      } catch (e) {
    ...
    function connectWebSocket() {
      ...
      const widgetToken = getWidgetTokenParam();
      const wsUrl = widgetToken
        ? `${protocol}//${window.location.host}?widgetToken=${encodeURIComponent(widgetToken)}`
    ...
    function setWidthClass(el, pct) {
      ...
      try {
        el.classList.forEach((c) => {
          if (c.startsWith('w-pct-')) el.classList.remove(c);
        });
      } catch {}
    ...
    function updateGoalDisplay(data, reason = '') {
      ...
      if (data.theme === 'modern-list') {
        ...
        if (!container) {
          ...
          if (titleEl)
            titleEl.textContent = customTitle.replace(/</g, '&lt;').replace(/>/g, '&gt;');
      ...
      if (isOBSWidget && data.theme !== 'modern-list') {
        setTipGoalVars({
          base: { bg: bgColor, border: borderColor, text: fontColor, progress: progressColor },
        });
      }
