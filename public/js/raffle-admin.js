  ...
  function updateUI(state) {
    ...
    try {
      ...
      if (commandInput && document.activeElement !== commandInput)
        commandInput.value = state.command || '!giveaway';
      if (prizeInput && document.activeElement !== prizeInput) prizeInput.value = state.prize || '';
      if (maxWinnersInput && document.activeElement !== maxWinnersInput)
        maxWinnersInput.value = state.maxWinners || 1;
      ...
      try {
        ...
        if (msg.type === 'init' && msg.raffle) {
          ...
          if (typeof winnerName === 'string' && winnerName) {
            const safeName = String(winnerName).replace(
              /[&<>"']/g,
              (m) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' })[m]
            );
