  ...
  const start = () => {
    ...
    async function loadSettings() {
      try {
        ...
        const url = token
          ? `/api/events-settings?token=${encodeURIComponent(token)}`
          : '/api/events-settings';
    ...
    function connectWebSocket() {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (
            (msg.type === 'tip' || msg.type === 'lastTip') &&
            settings.enabledActivities.includes('last-tip')
          ) {
          ...
          if (
            msg.type === 'last-achievement' &&
            settings.enabledActivities.includes('last-achievement')
          ) {
