...
export function initRaffle() {
  if (__raffle_started) return;
  __raffle_started = true;
  ...
  function renderRaffleContent() {
    ...
    if (
      ...
      const participantsHTML =
        '<div class="text-center text-gray-400"><span data-i18n="raffleNotConfigured"></span></div>';
    ...
    let winnerNames = [];
    if (Array.isArray(raffleData.winner)) winnerNames = raffleData.winner;
    else if (raffleData.winner) winnerNames = [raffleData.winner];
    ...
    const statusClass =
      state && state.paused ? 'raffle-status-badge paused' : 'raffle-status-badge active';
  ...
  function updateRaffleState(state, isRestore) {
    ...
    if (!isRestore && !raffleData.winner && state.enabled && (state.active || state.paused)) {
      localStorage.setItem(ACTIVE_STATE_KEY, JSON.stringify({ ...state, timestamp: Date.now() }));
