...
export async function initChat() {
  if (__chat_started) return;
  __chat_started = true;

  const start = async () => {
    const chatContainer = document.getElementById('chat-container');
    if (!chatContainer) {
      __chat_started = false;
      return;
    }
    let messageCount = 0;
    ...
    setInterval(async () => {
      try {
        ...
        if (
          cfg &&
          typeof cfg.avatarRandomBg === 'boolean' &&
          cfg.avatarRandomBg !== randomAvatarBgPerMessage
        ) {
          randomAvatarBgPerMessage = !!cfg.avatarRandomBg;
        }
      } catch {}
    ...
    async function loadTtsSettings() {
      try {
        ...
        if (s.ok) {
          ...
          console.log('Chat TTS settings:', j);
          ttsEnabled = !!j.ttsEnabled;
          ttsAllChat = !!j.ttsAllChat;
        }
        const l = await fetch('/api/tts-language');
        if (l.ok) {
          const j2 = await l.json();
          ttsLanguage = j2.ttsLanguage || 'en';
        }
      } catch {}
    }
    loadTtsSettings();
    ...
    function getNonce() {
      try {
        ...
        return (
          (m && (m.nonce || m.getAttribute('nonce'))) || document.head?.dataset?.cspNonce || ''
        );
      } catch {
        return '';
      }
    ...
    function setCssVar(name, value) {
      try {
        ...
        const updated = base.replace(/:root\s*\{([\s\S]*?)\}/, (m, body) => {
          const body2 = re.test(body)
            ? body.replace(re, decl)
            : decl
              ? body + (body.trim() ? ' ' : '') + decl
              : body;
          return `:root{${body2}}`;
    ...
    function updateDonationVars() {
      try {
        ...
        const anyThemeActive =
          (typeof serverHasTheme !== 'undefined' && serverHasTheme) ||
          !!(
            themeStyleTag &&
            typeof themeStyleTag.textContent === 'string' &&
            themeStyleTag.textContent.trim().length > 0
          );
    ...
    function setIfCustomVar(varName, value, defaultValue) {
      if (!varName) return;
      try {
        if (value && value !== defaultValue) setCssVar(varName, value);
        else setCssVar(varName, '');
      } catch {}
    }
    let originalAddMessage;
    function addMessage(msg) {
      /* placeholder, gets replaced below */
    }
    ...
    addMessage = function (msg) {
      ...
      const isLightThemeActive = !!(
        chatContainer && chatContainer.classList.contains('theme-light')
      );
    ...
    function addMessageImpl(msg) {
      ...
      const username = msg.channelTitle || 'Anonymous';
      {
        ...
        if (!hasCustom) {
          try {
            const bgIdx =
              Math.abs(username.split('').reduce((a, c) => c.charCodeAt(0) + ((a << 5) - a), 0)) %
              10;
            avatar.classList.add(`avatar-bg-${bgIdx}`);
          } catch {}
        }
        img.onerror = () => {
      ...
      if (msg.credits > 0) {
        const isDonationOnly =
          !normalText.length && !hasSticker && !/:([^\s:]+):/.test(cleanMessage);
      ...
      if (
        ...
        textElement.className =
          msg.credits > 0 ? 'message-text-inline has-donation' : 'message-text-inline';
        ...
        if (ttsAllChat && (normalText || cleanMessage) && msg.credits <= 0) {
      ...
      if (msg.credits > 0) {
        ...
        for (let i = 0; i < 200; i++) {
          ...
          const posClass = `pos-${Math.floor(Math.random() * 20)}`;
          confetti.classList.add(posClass);
          const sizeBucket = Math.floor(Math.random() * 5);
          confetti.classList.add(`size-${sizeBucket}`);
          const colorIdx = Math.floor(Math.random() * 6);
          confetti.classList.add(`color-${colorIdx}`);
          const durBucket = Math.floor(Math.random() * 9);
          confetti.classList.add(`dur-${durBucket}`);
          const delayBucket = Math.floor(Math.random() * 11);
          confetti.classList.add(`delay-${delayBucket}`);
      ...
      const isMinimalista =
        themeStyle &&
        typeof themeStyle.textContent === 'string' &&
        themeStyle.textContent.includes('THEME_ID:MINIMALISTA_AUTO10S');
      if (isMinimalista) {
        ...
        setTimeout(() => {
          ...
          if (
            isHorizontal ||
            (typeof animName === 'string' && animName.toLowerCase().includes('fadeout'))
          ) {
            if (messageEl.parentNode) messageEl.parentNode.removeChild(messageEl);
          }
        }, 11000);
    ...
    function formatText(text) {
      return formatWithMapping(text, EMOJI_MAPPING);
    }
    ...
    const isHorizontal =
      window.location.search.includes('horizontal=1') ||
      window.location.hash.includes('horizontal');
    if (isHorizontal) {
    ...
    async function fetchAndApplyTheme() {
      try {
        ...
        if (serverHasTheme) {
          ...
          const finalCss =
            serverCSS + extraCss + (donationOverrideCss ? `\n${donationOverrideCss}` : '');
          ...
          try {
            const localAuto = (localStorage.getItem('chatLiveThemeCSS') || '').match(
              /\/\* AUTO_FONT_SIZES_START \*\/[\u0000-\uFFFF]*?\/\* AUTO_FONT_SIZES_END \*\//
            );
          ...
          if (local && local !== lastThemeCSS) {
            ...
            let css = `
                    ...
                    `;
            const hasExplicitUserColors = !!(
              chatColors.usernameColor || chatColors.usernameBgColor
            );
            if (!hasExplicitUserColors) {
              css += CYBERPUNK_PALETTE.map(
                (p, i) =>
                  `\n                            .message-username.cyberpunk.cp-${i + 1} { color: ${p.text} !important; background: ${p.bg} !important; text-shadow: 0 0 8px ${p.border} !important; }`
              ).join('');
            } else {
      ...
      try {
        ...
        if (msg.type === 'ttsSettingUpdate') {
          if (Object.prototype.hasOwnProperty.call(msg.data || {}, 'ttsEnabled'))
            ttsEnabled = !!msg.data.ttsEnabled;
