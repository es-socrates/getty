document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    if (typeof randomAvatarBgPerMessage !== 'boolean' || randomAvatarBgPerMessage === false) {
      const res = await fetch(
        `/api/chat-config?nocache=${Date.now()}${token ? `&token=${encodeURIComponent(token)}` : ''}`
      );
  ...
  const wsUrl =
    ...
    (token ? `?ns=${encodeURIComponent(token)}` : '');
  let ws;
  ...
  async function loadColors() {
    ...
    try {
      const res = await fetch(
        `/api/modules?nocache=${Date.now()}${token ? `&token=${encodeURIComponent(token)}` : ''}`
      );
  ...
  function setIfCustomVar(varName, value, defaultValue) {
    try {
      setCssVar(varName, value && value !== defaultValue ? value : '');
    } catch {}
  }

  function setIfCustomInline() {
    /* deprecated */
  }
...

    {
      ...
      try {
        if (randomAvatarBgPerMessage) {
          ...
          bgIdxForThisMsg =
            Math.abs(username.split('').reduce((a, c) => c.charCodeAt(0) + ((a << 5) - a), 0)) % 10;
        }
      } catch {
        bgIdxForThisMsg = 0;
      }
      ...
      if (!hasCustom) {
        try {
          avatar.classList.add(`avatar-bg-${bgIdxForThisMsg}`);
        } catch {}
      }
  ...
  const isHorizontal =
    window.location.search.includes('horizontal=1') || window.location.hash.includes('horizontal');
  if (isHorizontal) {
