...
/*
  ...
  Usage examples:
    node scripts/hosted-verify.js --url https://app.getty.sh --token PUBLIC_TOKEN
    node scripts/hosted-verify.js --url https://app.getty.sh --token PUBLIC_TOKEN --admin ADMIN_TOKEN --start
    node scripts/hosted-verify.js --url https://app.getty.sh --token PUBLIC_TOKEN --admin ADMIN_TOKEN --start --draw
*/
...
async function verifyModules(baseUrl, token) {
  ...
  console.log('modules.lastTip.active =', !!lastTip.active);
  console.log('modules.lastTip.walletAddress =', lastTip.walletAddress ? '[set]' : '[empty]');
  console.log('modules.lastTip.lastDonation =', lastTip.lastDonation ? '[present]' : '[none]');
  console.log('modules.raffle =', Object.keys(raffle).length ? raffle : '[none]');
}
    ...
    ws.on('open', async () => {
      ...
      if (admin && start) {
        try {
          ...
          console.log('raffle/start -> OK');
        } catch (e) { console.log('raffle/start -> FAIL', e?.response?.status); }
      }
      if (admin && draw) {
        setTimeout(async () => {
          try {
            await http(`${baseUrl}/api/raffle/draw`, token, 'POST', {}, admin);
            console.log('raffle/draw -> OK');
          } catch (e) { console.log('raffle/draw -> FAIL', e?.response?.status); }
        }, 1000);
      }
    ...
    ws.on('message', (buf) => {
      try {
        ...
        if (msg.type === 'init' && msg.data) {
          ...
          if (ld) console.log('init.lastTip.lastDonation:', ld);
