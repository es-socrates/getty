...
describe('Additional API tests', () => {
  ...
  const res = await request(appRef).get('/api/tts-setting');
    expect(res.status).toBe(200);
    expect(typeof res.body.ttsEnabled).toBe('boolean');
  });

  test('POST /api/tts-setting validates payload', async () => {
  const bad = await request(appRef).post('/api/tts-setting').send({});
  expect(bad.status).toBe(200);
  expect(bad.body).toHaveProperty('ttsEnabled', false);

  const ok = await request(appRef).post('/api/tts-setting').send({ ttsEnabled: true });
  ...
  const res = await request(appRef).get('/api/tts-language');
    expect(res.status).toBe(200);
    expect(['en','es']).toContain(res.body.ttsLanguage);
  });

  test('POST /api/tts-language validates allowed values', async () => {
  const bad = await request(appRef).post('/api/tts-language').send({ ttsLanguage: 'fr' });
    expect(bad.status).toBe(400);

  const ok = await request(appRef).post('/api/tts-language').send({ ttsLanguage: 'es' });
    expect(ok.status).toBe(200);
    expect(ok.body).toHaveProperty('ttsLanguage', 'es');
  });

  test('GET /api/goal-audio-settings returns shape', async () => {
  const res = await request(appRef).get('/api/goal-audio-settings');
    expect(res.status).toBe(200);
  ...
  const res = await request(appRef).delete('/api/goal-audio-settings');
