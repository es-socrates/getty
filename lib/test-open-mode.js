...
function computeOpenTestMode() {
  ...
  const value = forceOpen ? true : (!requireSession && (
    process.env.GETTY_TEST_OPEN_MODE === '1' || autoCandidate
  ));
  if (process.env.NODE_ENV === 'test' && process.env.GETTY_TEST_OPEN_MODE_DEBUG === '1') {
    try {
      console.warn('[openTestMode]', JSON.stringify({
...
function isOpenTestMode() { return computeOpenTestMode(); }
