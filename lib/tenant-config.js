...
const enforceSecureKey = !allowInsecureKey
  && process.env.NODE_ENV !== 'test'
  && (process.env.REDIS_URL || process.env.GETTY_REQUIRE_SESSION === '1' || process.env.GETTY_MULTI_TENANT_WALLET === '1');

if (enforceSecureKey && (!process.env.STORE_ENCRYPTION_KEY || process.env.STORE_ENCRYPTION_KEY === DEFAULT_ENCRYPTION_KEY)) {
  throw new Error('[tenant-config] STORE_ENCRYPTION_KEY is required when running in hosted mode (Redis/session/wallet enabled). Set STORE_ENCRYPTION_KEY to a strong secret or opt-in with GETTY_ALLOW_INSECURE_STORE_KEY=1.');
}
