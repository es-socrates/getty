...
class TurboStorage {
  constructor(options = {}) {
    ...
    if (!key) {
      throw new Error(
        'Turbo storage not configured. Provide ARWEAVE_JWK_PATH or TURBO_ARWEAVE_JWK.'
      );
    }
    this.privateKey = key;
    this.provider = 'turbo';
  ...
  async uploadFile(bucket, fileName, fileData, options = {}) {
    ...
    if (buffer.length > MAX_FREE_SIZE_BYTES) {
      const error = new Error(`File too large for free upload. Maximum 100KB. Try using a smaller image or switch to Supabase storage.`);
      error.code = 'TURBO_FILE_TOO_LARGE';
      throw error;
    }

    const resolvedOptions = typeof options === 'string' ? { contentType: options } : { ...options };
    const contentType =
      resolvedOptions.contentType || resolvedOptions.mimeType || 'application/octet-stream';
    ...
    try {
      ...
      if (uploadError.message && uploadError.message.toLowerCase().includes('insufficient')) {
        const error = new Error('Upload not possible with Turbo. Please switch to Supabase storage.');
        error.code = 'TURBO_INSUFFICIENT_BALANCE';
        throw error;
      }
      if (uploadError.message && uploadError.message.toLowerCase().includes('size') || uploadError.message.toLowerCase().includes('large')) {
        const error = new Error(`File too large for free upload. Maximum 100KB. Try using a smaller image or switch to Supabase storage.`);
        error.code = 'TURBO_FILE_TOO_LARGE';
        throw error;
      }
