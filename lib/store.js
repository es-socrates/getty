...
class NamespacedStore {
  constructor({ redis, ttlSeconds = 72 * 3600 } = {}) {
    ...
    this.encryptionKey = this._deriveKey(process.env.STORE_ENCRYPTION_KEY || 'default-insecure-key-change-me');
  }
  ...
  _decrypt(encryptedText) {
    ...
    if (parts.length !== 2) return encryptedText; // Fallback for unencrypted data
