...
async function loadOwnerToken(store) {
  ...
  if (store && store.redis) {
    try {
      ...
      if (t && typeof t.token === 'string') return { token: t.token, source: 'store', claimed: true };
    } catch {}
  } else {
...
async function claimOwnerToken(store, provided) {
  ...
  const token = (typeof provided === 'string' && provided.trim().length >= 10) ? provided.trim() : generateToken();
  const entry = { token, createdAt: Date.now() };
  if (store && store.redis) {
    try { await store.set('global', 'owner-token', entry); return { ok: true, token, created: true }; } catch {}
    return { ok: false };
  }
...
async function rotateOwnerToken(store, oldToken) {
  ...
  if (store && store.redis) {
    try { await store.set('global', 'owner-token', entry); return { ok: true, token: newToken }; } catch {}
...
function extractOwnerTokenFromReq(req) {
  try {
    ...
    if (req.cookies && typeof req.cookies.ownerToken === 'string') return req.cookies.ownerToken.trim();
  } catch {}
  return null;
}
