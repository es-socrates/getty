...
function readHybridConfig(filePath) {
  try {
    ...
    try {
      ...
      return { data: {}, meta: { existed: true, corrupt: true } };
    }
    if (isWrapped(parsed)) {
      return {
    ...
    return {
      ...
      meta: { existed: true, wrapped: false, legacy: true },
    };
  } catch (e) {
...
function writeHybridConfig(filePath, dataObj) {
  ...
  const prevChecksum =
    prevMeta && prevMeta.checksum ? prevMeta.checksum : computeChecksum(prevData);
