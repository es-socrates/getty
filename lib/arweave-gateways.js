function baseGateways() {
  return [
    ...
    // Community / reliable
    'https://permagate.io',
    'https://arweave-search.knonce.com',
    'https://ar-io.dev',
    'https://ario-ny-mainnet.ams3.cdn.digitaloceanspaces.com', // CDN example (if it proxies GraphQL)
    'https://ar.io', // resolves -> ar-io network (redirects OK)
    // Additional public gateways (some may redirect)
    'https://arweave-search.jinnfi.workers.dev',
...
function applyEnv(list) {
  ...
  try {
    if (process.env.GETTY_ARWEAVE_EXTRA_GATEWAYS) {
      out.push(...process.env.GETTY_ARWEAVE_EXTRA_GATEWAYS.split(',').map(s=>s.trim()).filter(Boolean));
    }
  } catch {}
  try {
    if (process.env.GETTY_ARWEAVE_GATEWAY_EXCLUDE) {
      const excl = new Set(process.env.GETTY_ARWEAVE_GATEWAY_EXCLUDE.split(',').map(s=>s.trim()).filter(Boolean));
...
function buildGatewayList(opts = {}) {
  ...
  const shuffleTail = typeof opts.shuffleTail === 'boolean'
    ? opts.shuffleTail
