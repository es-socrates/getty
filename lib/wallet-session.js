function ensureWalletSession(req) {
  try {
    ...
    if (req.cookies && req.cookies.getty_wallet_session) raw = req.cookies.getty_wallet_session;
      ...
      try {
        ...
        for (const p of parts) {
          if (p.startsWith('getty_wallet_session=')) {
            raw = p.substring('getty_wallet_session='.length);
            break;
          }
        }
      } catch {}
    ...
    if (parsed && parsed.addr) {
      ...
      if (!req.tenant)
        req.tenant = { walletAddress: parsed.addr, walletHash: deriveWalletHash(parsed.addr) };
      return parsed;
    }
  } catch {}
  return null;
