...
function normalizeMessageEntry(raw = {}) {
  return {
    ...
    imageStorageProvider:
      typeof raw.imageStorageProvider === 'string' ? raw.imageStorageProvider : '',
      ...
      return {
        messages: Array.isArray(raw.messages)
          ? raw.messages.map((msg) => ({
              ...
              imageStorageProvider:
                typeof msg?.imageStorageProvider === 'string' ? msg.imageStorageProvider : '',
              imageStoragePath:
                typeof msg?.imageStoragePath === 'string' ? msg.imageStoragePath : '',
              imageSha256: typeof msg?.imageSha256 === 'string' ? msg.imageSha256 : '',
              imageFingerprint:
                typeof msg?.imageFingerprint === 'string' ? msg.imageFingerprint : '',
              imageOriginalName:
                typeof msg?.imageOriginalName === 'string' ? msg.imageOriginalName : '',
            }))
          : [],
        cooldownSeconds: Number(raw.cooldownSeconds) > 0 ? Number(raw.cooldownSeconds) : 300,
        theme: 'horizontal',
        bgColor:
          typeof raw.bgColor === 'string' && /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(raw.bgColor)
            ? raw.bgColor
            : '#0e1014',
        textColor:
          typeof raw.textColor === 'string' &&
          /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(raw.textColor)
            ? raw.textColor
            : '#e8eef2',
        animationMode: ['fade', 'slide-up', 'slide-left', 'scale', 'random'].includes(
          raw.animationMode
        )
          ? raw.animationMode
          : 'fade',
        defaultDurationSeconds:
          Number(raw.defaultDurationSeconds) >= 1 && Number(raw.defaultDurationSeconds) <= 60
            ? Number(raw.defaultDurationSeconds)
            : 10,
        staticMode: raw.staticMode === true,
        bannerBgType:
          raw.bannerBgType === 'gradient' || raw.bannerBgType === 'solid'
            ? raw.bannerBgType
            : 'solid',
        gradientFrom:
          typeof raw.gradientFrom === 'string' &&
          /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(raw.gradientFrom)
            ? raw.gradientFrom
            : '#4f36ff',
        gradientTo:
          typeof raw.gradientTo === 'string' &&
          /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(raw.gradientTo)
            ? raw.gradientTo
            : '#10d39e',
      };
    }
...
class AnnouncementModule {
  ...
  async _getState(ns) {
    ...
    if (this.store) {
      try {
        const j =
          (await this.store.getConfig(ns, 'announcement-config.json', null)) ||
          (await this.store.get(ns, 'announcement-config', null));
  ...
  stop(ns = null) {
    if (ns) {
      ...
      for (const [k, t] of this._timers.entries()) {
        try {
          clearTimeout(t);
        } catch {}
  ...
  async broadcastRandomMessage(ns = null) {
    ...
    if (process.env.NODE_ENV === 'test') {
      log.debug('broadcastRandomMessage %j', {
        ns: ns || null,
        pool: pool.length,
        pick: msg && msg.id,
      });
      ...
      if (this.store && (ns || ns === null)) {
        ...
        if (lt && lt.data) {
          ...
          if (
            ...
            (wrapped.__version || wrapped.checksum || wrapped.updatedAt)
          ) {
            meta = {
  ...
  async setSettings(
    ...
    if (typeof bgColor === 'string' && /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(bgColor))
      st.bgColor = bgColor;
    if (typeof textColor === 'string' && /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(textColor))
      st.textColor = textColor;
    if (['fade', 'slide-up', 'slide-left', 'scale', 'random'].includes(animationMode))
      st.animationMode = animationMode;
    ...
    if (typeof gradientFrom === 'string' && /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(gradientFrom))
      st.gradientFrom = gradientFrom;
    if (typeof gradientTo === 'string' && /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(gradientTo))
      st.gradientTo = gradientTo;
  ...
  async updateMessage(id, patch, ns = null) {
    ...
    if (patch.durationSeconds !== undefined) {
      ...
      if (d >= 1 && d <= 60) next.durationSeconds = d;
    ...
    if (Object.prototype.hasOwnProperty.call(patch, 'titleSize'))
      next.titleSize = clamp(patch.titleSize, 8, 72);
    if (Object.prototype.hasOwnProperty.call(patch, 'subtitle1Size'))
      next.subtitle1Size = clamp(patch.subtitle1Size, 8, 64);
    if (Object.prototype.hasOwnProperty.call(patch, 'subtitle2Size'))
      next.subtitle2Size = clamp(patch.subtitle2Size, 8, 64);
    if (Object.prototype.hasOwnProperty.call(patch, 'subtitle3Size'))
      next.subtitle3Size = clamp(patch.subtitle3Size, 8, 64);
    if (Object.prototype.hasOwnProperty.call(patch, 'textSize'))
      next.textSize = clamp(patch.textSize, 8, 64);
    if (Object.prototype.hasOwnProperty.call(patch, 'ctaTextSize'))
      next.ctaTextSize = clamp(patch.ctaTextSize, 8, 64);
    if (Object.prototype.hasOwnProperty.call(patch, 'imageLibraryId'))
      next.imageLibraryId = typeof patch.imageLibraryId === 'string' ? patch.imageLibraryId : '';
    if (Object.prototype.hasOwnProperty.call(patch, 'imageStorageProvider'))
      next.imageStorageProvider =
        typeof patch.imageStorageProvider === 'string' ? patch.imageStorageProvider : '';
    if (Object.prototype.hasOwnProperty.call(patch, 'imageStoragePath'))
      next.imageStoragePath =
        typeof patch.imageStoragePath === 'string' ? patch.imageStoragePath : '';
    if (Object.prototype.hasOwnProperty.call(patch, 'imageSha256'))
      next.imageSha256 = typeof patch.imageSha256 === 'string' ? patch.imageSha256 : '';
    if (Object.prototype.hasOwnProperty.call(patch, 'imageFingerprint'))
      next.imageFingerprint =
        typeof patch.imageFingerprint === 'string' ? patch.imageFingerprint : '';
    if (Object.prototype.hasOwnProperty.call(patch, 'imageOriginalName'))
      next.imageOriginalName =
        typeof patch.imageOriginalName === 'string' ? patch.imageOriginalName : '';
    if (!next.imageUrl) {
      next.imageLibraryId = '';
      next.imageStorageProvider = '';
      next.imageStoragePath = '';
  ...
  async removeMessage(id, ns = null) {
    ...
    if (removed && removed.imageUrl && removed.imageUrl.startsWith('/uploads/announcement/')) {
      try {
        fs.unlinkSync(path.join(process.cwd(), 'public', removed.imageUrl));
      } catch {}
    }
    await this._saveState(ns, st);
    await this.broadcastConfig(ns);
  ...
  async clearMessages(mode = 'all', ns = null) {
    ...
    if (mode === 'test') {
      const testRegex =
        /^(Default dur|DD-|WS ping|Image test message updated|Temp img message updated)/;
