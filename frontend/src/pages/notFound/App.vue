<template>
  <div class="welcome-page not-found-page">
    <header class="header">
      ...
      <a
        ...
        <span class="btn-label" data-default-label="Home">Home</span>
      </a>
      <button id="theme-toggle" class="theme-toggle" title="Toggle theme" aria-label="Toggle theme">
        ...
        <svg class="moon-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          <path
    ...
    <div class="wrap">
      ...
      <p id="welcome-status" class="welcome-status hidden" role="status" aria-live="polite"></p>
    </div>
  </div>
</template>
...
function notifyLegacyBridge() {
  ...
  window.dispatchEvent(
    new CustomEvent('getty-welcome-vue-ready', {
      detail: { root },
    })
  );
  if (typeof window.__GETTY_WELCOME_VUE_READY__ === 'function') {
    try {
      window.__GETTY_WELCOME_VUE_READY__();
    } catch (error) {
      console.warn('[404] failed to notify legacy bridge', error);
    }
  }
}
