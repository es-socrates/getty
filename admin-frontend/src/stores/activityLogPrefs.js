...
function load(){
  ...
  try {
    ...
    if (raw){
      ...
      if (typeof obj.autoScrollDefault === 'boolean') autoScrollDefault.value = obj.autoScrollDefault;
...
function save(){
  try {
    const obj = { enabled: enabled.value, collapsed: collapsed.value, autoScrollDefault: autoScrollDefault.value, limitDefault: limitDefault.value };
