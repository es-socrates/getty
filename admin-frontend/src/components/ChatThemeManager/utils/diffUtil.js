function computeLineDiff(aText, bText) {
  ...
  while (i < m) { out.push({ text: a[i], type: 'del' }); i++; }
  while (j < n) { out.push({ text: b[j], type: 'add' }); j++; }
  return out;
}
...
try { if (typeof module !== 'undefined' && module.exports) { module.exports = { computeLineDiff, default: { computeLineDiff } }; } } catch {}
