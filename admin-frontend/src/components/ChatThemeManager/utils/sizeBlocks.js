const SizeBlocks = {
  clamp(v) { return Math.min(60, Math.max(1, v)); },
  build(fontSizes) {
  ...
  extract(css) {
    const sizes = {}; if (!css) return sizes;
    const re = {
    ...
    for (const k in re) { const m = css.match(re[k]); if (m) sizes[k] = Number(m[1]); }
    return sizes;
  }
};
...
try { if (typeof module !== 'undefined' && module.exports) { module.exports = { SizeBlocks, default: { SizeBlocks } }; } } catch {}
