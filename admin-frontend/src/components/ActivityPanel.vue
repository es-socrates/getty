  ...
  <OsCard :title="t('activityLogTitle') || 'Activity Log'">
    <div class="flex flex-wrap items-center gap-2 mb-2">
      ...
      <div class="flex items-center gap-2">
        ...
        <input v-model="q" placeholder="keyword" class="px-2 py-1 rounded-os-sm bg-[var(--bg-chat)] border border-[var(--card-border)] text-sm w-44" />
      </div>
    ...
    <div class="flex flex-wrap gap-2 mb-2" v-if="chips.length">
      <span v-for="(chip, i) in chips" :key="i" class="inline-flex items-center gap-2 px-2 py-0.5 rounded-md border border-[var(--card-border)] bg-[var(--bg-chat)] text-xs">
    ...
    <div ref="listRef" class="os-subtle rounded-os-sm max-h-72 overflow-auto" aria-live="polite">
      <div v-for="(it, idx) in items" :key="idx" class="px-2 py-1 text-xs border-b flex items-center gap-2">
        <span :class="badgeClass(it.level)" class="inline-block px-1 rounded">{{ it.level.toUpperCase() }}</span>
