    ...
    <OsCard class="mb-4">
      ...
      <div class="grid gap-3 [grid-template-columns:repeat(auto-fill,minmax(200px,1fr))]">
        <div
          ...
          :key="m.key"
          class="os-subtle p-3 rounded-os-sm"
          :title="m.tooltip || ''">
          <div class="flex items-center gap-2 font-semibold text-sm">
            <span
              :class="[
                'w-2 h-2 rounded-full',
                m.active ? 'bg-[#16a34a]' : 'bg-neutral-400',
              ]"></span>
            {{ m.label }}
            <span
              v-if="m.hasUptime"
              class="ml-auto inline-flex items-center gap-1 text-[11px] text-neutral-400"
              :title="m.activeSince || ''">
              <svg
                width="14"
                height="14"
    ...
    <OsCard class="mb-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <div class="p-3 os-subtle rounded-os-sm">
          ...
          <div
            class="inline-flex items-center px-2 py-0.5 rounded-md border border-[var(--card-border)] bg-[var(--bg-chat)] text-xs font-mono uppercase tracking-wide">
            {{ system?.env || '—' }}
          </div>
        </div>
      </div>
...
const showTokenNudge = computed(() => {
  try {
    ...
    return (
      ...
      (!sessionStatus.value.active || !lastPublicToken.value) &&
      !dismissed
    );
  } catch {
    return (
...
function dismissNudge() {
  try {
    localStorage.setItem('getty_token_nudge_v1', '1');
  } catch {}
}
...
function onMaybeLater() {
  dismissNudge();
  try {
    pushToast({ i18nKey: 'tokenNudgeDismissed', type: 'info', timeout: 2500 });
  } catch {}
}
...
async function load() {
  try {
    ...
    const toUptime = (obj) => {
      ...
      const pct = Math.min(Math.max((secs / 3600) * 100, 0), 100);
      return {
        hasUptime: secs > 0,
